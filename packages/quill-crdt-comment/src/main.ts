...
import type { Range } from "./layer";
import { initLayerDOM, renderLayer } from "./layer";
...
const renderAllCommentWithRelativePosition = () => {
  ...
  for (const item of COMMENT_LIST) {
    ...
    const stratPosition = Y.createAbsolutePositionFromRelativePosition(start, ydoc);
...
applyComment.onclick = () => {
  ...
  if (selection) {
    ...
    const end = Y.createRelativePositionFromTypeIndex(ytext, sel.index + sel.length);
